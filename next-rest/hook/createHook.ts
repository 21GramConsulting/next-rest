...
import { UseHook } from '#rest/hook/UseHook';
import { Identifiable, Identified, isUnidentified } from '#rest/Identifiable';
import { Query as QueryOf } from '#rest/Query';
import { isId, isQueryDefined, isSelection, Selection as SelectionType } from '#rest/hook/Selection';
import { Hook } from '#rest/hook/Hook';
import { ___endpoint } from '#rest/___endpoint';
import useSWR from 'swr';
import fetch from '#rest/hook/fetch';
...
// @formatter:on
  ...
  function useHook(selection: any): any {
    ...
    const outputReader = (uri: string) => fetch(uri)
      .then(
        r => isSelection(selection)
    ...
    async function write(value: Set<Record>): Promise<Set<Record & Identified<ID>>>;
    async function write(value: any): Promise<any> {
      if (value instanceof Set) return new Set(
        await Promise.all(Array.from(value).map(write)),
      );
    ...
    async function remove(value: Record & Identified<ID> | Set<Record & Identified<ID>>) {
